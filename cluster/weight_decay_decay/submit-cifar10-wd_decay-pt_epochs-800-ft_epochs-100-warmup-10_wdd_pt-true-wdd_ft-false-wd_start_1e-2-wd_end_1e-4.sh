...
#SBATCH --gres=gpu:1
#SBATCH --job-name=wd_decay-cifar10-pt_epochs-800-ft_epochs-100-warmup-10-wdd_pt-true-wdd_ft-false-wd_start_1e-2-wd_end_1e-4
#SBATCH -o /home/ferreira/workspace/experiments/metassl/logs/%x.%N.%A.%a.out
#SBATCH --array=0-3%1
#SBATCH --exclude=dlcgpu03

TRAIN_EPOCHS=800
FINETUNING_EPOCHS=100
WARMUP_EPOCHS=10
EXPT_NAME="wd_decay-cifar10-pt_epochs-800-ft_epochs-100-warmup-10-wdd_pt-true-wdd_ft-false-wd_start_1e-2-wd_end_1e-4"
...
srun $WORKFOLDER/cluster/weight_decay_decay/cifar_train_finetune_wdd_pt_true_wdd_ft_false-wd_start_1e-2-wd_end_1e-4.sh $EXPT_NAME $TRAIN_EPOCHS $FINETUNING_EPOCHS $WARMUP_EPOCHS $CONFIG
