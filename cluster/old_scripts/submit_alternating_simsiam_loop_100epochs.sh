...
EXPT_NAME="alternating-advanced-stats-ftlr-100-ptlr-5e-2-bs-256-epochs-100"
#EXPT_NAME="test"
...
# hostname
timeout 23h srun -p $PARTITION --exclude=dlcgpu24,dlcgpu22 --gres=gpu:8 --job-name=$EXPT_NAME -o /home/ferreira/workspace/experiments/metassl/logs/%x.%N.%A.%a.out $WORKFOLDER/cluster/train_alternating_simsiam.sh $EXPT_NAME $TRAIN_EPOCHS $FINETUNING_EPOCHS
if [[ $? -eq 124 ]]; then
	# echo "timeout"
	# hostname
	$WORKFOLDER/cluster/submit_alternating_simsiam_loop_100epochs.sh
fi
