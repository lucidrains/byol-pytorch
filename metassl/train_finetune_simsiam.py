...
def main_worker(gpu, ngpus_per_node, config, expt_dir):
    ...
    for epoch in range(config.train.start_epoch, config.train.epochs):
        ...
        if epoch % 2 == 0:
            ...
            layers_to_retain_ft = prepare_pretraining(model, config, ngpus_per_node, layers_to_retain_pt=layers_to_retain_pt)
            print(layers_to_retain_ft)
            pretrain(pt_train_loader, model, criterion, pt_optimizer, epoch, config, test_mode=True)
            ...
            layers_to_retain_pt = prepare_finetuning(model, config, ngpus_per_node, layers_to_retain_ft=layers_to_retain_ft)
            print(layers_to_retain_pt)
            
            # todo: write finetune(), evaluate()
...
def prepare_finetuning(model, config, ngpus_per_node, layers_to_retain_ft=None):
    if config.expt.rank % ngpus_per_node == 0:
        ...
        model.module.encoder.fc = nn.Linear(2048, 1000).cuda()
        
        # init or load the fc layer
        if layers_to_retain_ft:
            model.module.encoder.fc.weight.copy_(layers_to_retain_ft['module.encoder.fc.weight']).cuda()
            model.module.encoder.fc.bias.copy_(layers_to_retain_ft['module.encoder.fc.bias']).cuda()
        else:
...
def prepare_pretraining(model, config, ngpus_per_node, layers_to_retain_pt=None):
    layers_to_retain_ft = None
    if config.expt.rank % ngpus_per_node == 0:
        if layers_to_retain_pt:
            ...
            with torch.no_grad():
                print(layers_to_retain_pt['module.encoder.fc.0.weight'])
                model.module.encoder.fc[0].weight.copy_(layers_to_retain_pt['module.encoder.fc.0.weight']).cuda()
                
                model.module.encoder.fc[1].weight.copy_(layers_to_retain_pt['module.encoder.fc.1.weight']).cuda()
                model.module.encoder.fc[1].bias.copy_(layers_to_retain_pt['module.encoder.fc.1.bias']).cuda()
                
                model.module.encoder.fc[3].weight.copy_(layers_to_retain_pt['module.encoder.fc.3.weight']).cuda()
                
                model.module.encoder.fc[4].weight.copy_(layers_to_retain_pt['module.encoder.fc.4.weight']).cuda()
                model.module.encoder.fc[4].bias.copy_(layers_to_retain_pt['module.encoder.fc.4.bias']).cuda()
                
                model.module.encoder.fc[6].weight.copy_(layers_to_retain_pt['module.encoder.fc.6.weight']).cuda()
                
                model.module.encoder.fc[6].bias.copy_(layers_to_retain_pt['module.encoder.fc.6.bias']).cuda()
